...
func spawn_multiple_enemies(enemy, path_id, amount, delay, spacing: float, difficulty = 0, order = 0):
	#print(float(order), " ", float(amount))
	spawn_enemy(enemy, path_id, 1.0 - spacing*float(order), difficulty)
	...
	var timer = Timer.new()
	add_child(timer)
	var callable = Callable(self, "spawn_multiple_enemies").bind(enemy, path_id, amount - 1,
		delay, spacing, difficulty, order + 1)
